...
class Block(gameobject.GameObject):
    ...
    def wait(self):
        if self.block_below != None:
            if self.rect.bottom < blockgrid.RECT.bottom and isinstance(self.block_below, Block) and self.rect.bottom > self.block_below.rect.top:  #If within the grid and there is no block below
                self.acceleration[1] = GRAVITY
                self.state           = STATES.FALLING
