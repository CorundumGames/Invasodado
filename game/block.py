...
class Block(GameObject):
    ...
    def __init__(self, position, newcolor = random.choice(color.LIST), special = False):
        ...
        self.rect     = pygame.Rect([round(position[0] / size[0]) * size[0],
    ...
    def appear(self):
        ...
        self.emitter      = ParticleEmitter(block_particles[id(self.color)], self.rect, 5, ingame.BLOCKS)
    ...
    def start_falling(self):
        ...
        for i in xrange(gc[0] + 1, blockgrid.DIMENSIONS[0]):
        #For all grid cells below this one...
                if isinstance(bl[i][gc[1]], Block) and (bl[i][gc[1]].state is not Block.STATES.FALLING):
                #If this grid cell is occupied...
                    self.target = i - 1
                    break

        if gc[0] > 0 and isinstance(bl[gc[0] - 1][gc[1]], Block):
        #If there is a block above us...
            for i in xrange(gc[0], 0, -1):
                if isinstance(bl[i][gc[1]], Block):
                    bl[i][gc[1]].state = Block.STATES.START_FALLING
                else:
                    break
    ...
    def wait(self):
        ...
        and (bl[gc[0] + 1][gc[1]] is None or \
             bl[gc[0] + 1][gc[1]].state is Block.STATES.FALLING):
    ...
    def fall(self):
        ...
        r.top             = self.position[1] + .5  #Round to the nearest integer
        gc[0]             = r.centery / r.height
        self.emitter.rect.topleft = r.topleft
        ...
        while self.target >= 0 and isinstance(bl[self.target][gc[1]], Block) \
        and not (bl[self.target][gc[1]].state is Block.STATES.FALLING):
        ...
        if gc[0] == blockgrid.DIMENSIONS[0] - 1 or isinstance(bl[gc[0] + 1][gc[1]], Block):
        #If we've hit something...
            if gc[0] == blockgrid.DIMENSIONS[0] - 1:
            #If that something was the grid's bottom...
                r.bottom = blockgrid.RECT.bottom
            else:
            #Otherwise it was another block.
                r.bottom = bl[gc[0] + 1][gc[1]].rect.top
    ...
    def stop(self):
        ...
        self.velocity[1]               = 0.0
        gc[0]                          = self.rect.centery / self.rect.height #(row, column)
        blockgrid.blocks[gc[0]][gc[1]] = self

        self.target = None
        blockgrid.blockstocheck.add(self)
        bump.play()
        self.snap()
        ...
        #If this is a special block...
            ...
            #If we're not at the bottom of the grid...
                self.color = blockgrid.blocks[gc[0] + 1][gc[1]].color
                blockgrid.clear_color(self.color)
            else:
                blockgrid.clear_row(gc[0])
