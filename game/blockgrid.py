...
blocks          = [[None] * DIMENSIONS[1]] * DIMENSIONS[0]
block_type      = None
blocks_to_check = set()
blocks_to_clear = set()
...
def clear_row(row):
    ...
    @postcondition: No blocks exist in the given row.
    '''
...
def update():
    '''
    Checks to see if any blocks of the same color are lined up, clears them if so.

    @postcondition: Sets of 3+ like-colored blocks are removed from the game field.
    '''
    global blocks
    blocks = [[None] * DIMENSIONS[1]] * DIMENSIONS[0]

    for i in (j for j in chain.from_iterable(blocks) if j and j.state == block_type.STATES.ACTIVE):
    #For all blocks that are on the grid and not moving...
        assert isinstance(i, block_type), \
        "A %s somehow got into the Block grid!" % i
